      ...
      <template v-slot:top>
        ...
        >
          ...
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              <v-card-title class="text-h5">¿Quieres eliminar este estadio?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'Nuevo Estadio' : 'Actualizar Estadio'
    },
  },
  ...
  methods: {
    listar(){
        ...
        axios.get('/estadio').then((response) =>{
            me.estadios = response.data;
            console.log(me.estadios);
        }).catch((error)=>{
             console.log(error);
        });
    },
    ...
    deleteItemConfirm () {
      let me = this;
      axios.delete('/estadio/delete/'+me._id).then((response) =>{
          me.close();
          me.listar();
          me.limpiar();
          me.closeDelete()
